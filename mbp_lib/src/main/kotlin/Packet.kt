...
data class DataPacketBody(val data: ByteArray) : PacketBody() {
    override fun equals(other: Any?): Boolean =
        (other is DataPacketBody) and (data.contentEquals((other as DataPacketBody).data))
...
data class FinalizePacketBody(val murmurHash3: ByteArray) : PacketBody() {
    override fun equals(other: Any?): Boolean =
        (other is FinalizePacketBody) and (murmurHash3.contentEquals((other as FinalizePacketBody).murmurHash3))
